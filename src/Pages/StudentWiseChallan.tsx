...
const StudentWiseChallan: React.FC<AllProps> = ({ onLogout, isOpen = false }) => {
  ...
  const renderErrorMessages = () => {
    const errorMessages = [];
    if (formErrors.period) errorMessages.push("Period is Required");
    if (formErrors.feeFrom) errorMessages.push("Fee From is Required");
    if (formErrors.feeTo) errorMessages.push("Fee To is Required");
    if (formErrors.stdName) errorMessages.push("Student Name is Required");
  ...
  return (
    <div className="m-auto">
      <Navbar title="Studentwise Challan" onLogout={onLogout} />
      <div className="p-5">
            ...
            <div className="border border-customBlue bg-[#DEEBF4] rounded p-5">
              <div className={
                ...
                <div className="col-span-1 flex items-center">
                  <input
                    name="emptyinput"
                    className="flex-grow lg:px-3 px-1 md:p-1 border border-customBlue focus:bg-gray-200 rounded focus:outline-none active:border-blue-500"
                    type="text"
                  />
                </div>
              </div>
              ...
              <div className="flex justify-end gap-3 mt-5">
                <button
                  type="button"
                  onClick={handleSave}
                  className="px-1 py-1 w-[100px] text-sm md:text-base lg:text-lg border border-customBlue rounded font-medium shadow-md hover:bg-blue-800 bg-white text-black hover:text-white active:shadow-inner transform active:translate-y-0.5"
                >
                  View
                </button>
                <button className="px-1 py-1 w-[100px] text-sm md:text-base lg:text-lg border border-customBlue rounded font-medium shadow-md hover:bg-blue-800 bg-white text-black hover:text-white active:shadow-inner transform active:translate-y-0.5">
                  Exit
                </button>
              </div>
            </div>
...
export default StudentWiseChallan;